...
function writeIn(btnText) {
  ...
  if (fieldText.length === 19) {} else {
    ...
    if ((lastChar === "+" || lastChar === "*" || lastChar === "/" || lastChar === "-" || lastChar === "%") && (strText === "+" || strText === "*" || strText === "/" || strText === "%"))
      ...
      fieldText = fieldText.substring(0, fieldText.length - 1);
    if ((last2Char === "+-" || last2Char === "*-" || last2Char === "/-" || last2Char === "%-") && (strText === "+" || strText === "-" || strText === "*" || strText === "/" || strText === "%")) {} else if (fieldText === "0") {
      fieldText = "";
      fieldText += strText;
      memory = fieldText;
...
function equals(str) {
  ...
  var numDec = nums1.filter(function(val) {
    ...
    return (pers != null && pers.length > 1);
  });
